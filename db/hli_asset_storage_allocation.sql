...
CREATE TABLE `storage_allocation` (
  ...
  `action_type` enum('할당','회수','증설','축소','기타') NOT NULL,
  `action_date` date NOT NULL,
  `target_storage` varchar(200) NOT NULL,
  `target_system` varchar(200) NOT NULL,
  `capacity_change` float DEFAULT NULL,
  ...
  CONSTRAINT `storage_allocation_ibfk_1` FOREIGN KEY (`storage_id`) REFERENCES `storage_info` (`id`) ON DELETE CASCADE
